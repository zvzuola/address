  ...
  <section>
    ...
    >
      ...
      >
        ...
        <i :class="$style['icon-arrow']" v-show="index===0"></i>
      </div>
    </div>
    <!-- 城市切换 -->
    <city-change v-show="toolPanelVisible[0]" :class="$style['tool-panel-city']"></city-change>
  ...
  methods: {
    handleToolBtnClick(index) {
      if(index < this.toolPanelVisible.length){
        this.toolPanelVisible[index] = this.toolPanelVisible[index] == 1 ? 0 : 1;
